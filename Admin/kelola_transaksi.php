...
// 🔒 Handle approve/reject return request - FIXED
if(isset($_POST['process_return'])) {
    $transaction_id = (int)$_POST['transaction_id'];
    $action = $_POST['process_return']; // FIX: name tombol adalah process_return
...
try {
    ...
    foreach($transactions as $trx) {
        ...
        $return_reason = $trx['return_reason'] ?? '';
        $courier = $trx['courier'] ?? null;
        $service_type = $trx['service_type'] ?? null;
        $notes = $trx['notes'] ?? '';
        ...
        $transactionDetails[$trx['id']] = [
            ...
            'return_reason' => nl2br(htmlspecialchars($return_reason)),
            'courier' => $courier,
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .courier-badge { display: inline-block; background: #dbeafe; color: #1e40af; padding: 4px 10px; border-radius: 6px; font-size: 13px; font-weight: 600; margin-top: 5px; }
    </style>
</head>
<body>
    <div class="container">
    ...
    <script>
        ...
        function viewDetail(transactionId) {
            ...
            let courierHTML = '';
            if(details.courier || details.service_type) {
                const courierText = details.courier ? details.courier.toUpperCase() : '-';
                const serviceText = details.service_type ? '- ' + details.service_type.replace('_', ' ').toUpperCase() : '';
                courierHTML = `<div class="detail-row" style="align-items: flex-start;"><span class="detail-label">Opsi Pengiriman:</span><span class="detail-value" style="text-align: right;"><span class="courier-badge">${courierText} ${serviceText}</span></span></div>`;
            }
            
            let returnSection = '';
            const rs = details.return_status;
            if(rs === 'requested') {
                returnSection = `
                <div style="margin-top:20px;padding:15px;background:#fffbeb;border:1px solid #f59e0b;border-radius:8px;">
                    ...
                    <form method="POST" id="returnForm" style="display:flex;flex-direction:column;gap:10px;">
                        ...
                        <textarea name="admin_note" placeholder="Catatan admin (opsional)" style="padding:10px;border:1px solid #e5e7eb;border-radius:6px;min-height:60px;"></textarea>
            ...
            const statusBadgeClass = rs !== 'none' ? 'return_' + rs : details.status;
            const statusBadgeText = rs === 'approved' ? 'Pembatalan Disetujui' :
                                   (rs === 'requested' ? 'Pembatalan Diajukan' :
                                   (rs === 'rejected' ? 'Pembatalan Ditolak' :
            ...
            document.getElementById('detailContent').innerHTML = `
                <div class="transaction-detail">
                    ...
                    ${courierHTML}
                    <div class="detail-row" style="align-items: flex-start;"><span class="detail-label">Catatan Pesanan:</span><span class="detail-value" style="text-align: right; font-style: italic; color: #6b7280;">${details.notes}</span></div>
                    ${returnSection}
                </div>
